...
class USWeatherOutput(BaseModel):
    ...
    is_weather_in_us: bool = Field(
        ...,
        description="True if the user is asking about weather in the US, False otherwise.",
    )
    is_weather: bool = Field(
        ..., description="True if the user is asking about weather, False otherwise."
    )
...
async def us_weather_guardrail(
    ...
    return GuardrailFunctionOutput(
        ...
        tripwire_triggered=(
            not (result.final_output.is_weather_in_us)
            and result.final_output.is_weather
        ),
    )

...
def build_client(
    *, timeout: httpx.Timeout = DEFAULT_TIMEOUT, headers: Optional[Dict[str, str]] = None
) -> httpx.AsyncClient:
    return httpx.AsyncClient(timeout=timeout, headers={**DEFAULT_HEADERS, **(headers or {})})

async def _get_json(
    client: httpx.AsyncClient, url: str, params: Dict[str, Any], *, retries: int = 2, backoff: float = 0.6
) -> Dict[str, Any]:
...
async def get_current_weather(
    lat: float, lon: float, *, timezone: str = "auto", client: Optional[httpx.AsyncClient] = None
) -> Dict[str, Any]:
    close = False
    if client is None:
    ...
    try:
        data = await _get_json(
            client, f"{API_BASE}{FORECAST_PATH}",
            {"latitude": lat, "longitude": lon, "current_weather": True, "timezone": timezone}
        )
...
async def get_hourly_forecast(
    ...
    hourly: Optional[Union[str, Iterable[str]]] = None, limit_hours: Optional[int] = None,
    client: Optional[httpx.AsyncClient] = None
) -> Dict[str, Any]:
    fields = _as_list(hourly) or ["temperature_2m","precipitation","windspeed_10m","weathercode"]
    close = False
    if client is None:
        client, close = build_client(), True
    try:
        ...
        if limit_hours and "time" in hourly_block:
            ...
            for k, v in list(hourly_block.items()):
                if isinstance(v, list): hourly_block[k] = v[:n]
        return hourly_block
    finally:
        if close: await client.aclose()

async def get_daily_forecast(
    lat: float, lon: float, *, days: int = 3, timezone: str = "auto",
    daily: Optional[Union[str, Iterable[str]]] = None, client: Optional[httpx.AsyncClient] = None
) -> Dict[str, Any]:
    metrics = _as_list(daily) or ["temperature_2m_max","temperature_2m_min","precipitation_sum"]
    close = False
    if client is None:
        client, close = build_client(), True
    try:
        data = await _get_json(
            client, f"{API_BASE}{FORECAST_PATH}",
            {
                "latitude": lat, "longitude": lon, "daily": metrics,
                "forecast_days": max(1, int(days)), "timezone": timezone
            }
        )
...
async def get_air_quality(
    ...
    if h: params["hourly"] = h
    if c: params["current"] = c
    close = False
    if client is None:
        client, close = build_client(), True
    try:
...
async def get_marine_forecast(
    lat: float, lon: float, *, timezone: str = "auto",
    hourly: Optional[Union[str, Iterable[str]]] = None, client: Optional[httpx.AsyncClient] = None
) -> Dict[str, Any]:
    fields = _as_list(hourly) or ["wave_height","wave_direction","wave_period","wind_wave_height"]
    close = False
    if client is None:
        client, close = build_client(), True
    try:
        return await _get_json(
            client, f"{MARINE_BASE}{MARINE_PATH}",
            {"latitude": lat, "longitude": lon, "hourly": fields, "timezone": timezone}
        )
    finally:
        if close: await client.aclose()
...
async def geocode_search(
    name: str, *, count: int = 10, language: Optional[str] = None, client: Optional[httpx.AsyncClient] = None
) -> Dict[str, Any]:
...
async def get_historical_weather(
    ...
    hourly: Optional[Union[str, Iterable[str]]] = None, timezone: str = "auto",
    client: Optional[httpx.AsyncClient] = None
) -> Dict[str, Any]:
    ...
    params = {
        ...
        "hourly": fields, "timezone": timezone
    }
    close = False
    if client is None:
        client, close = build_client(), True
...
async def get_historical_forecast(
    ...
    if client is None:
        client, close = build_client(), True
    try:
        return await _get_json(client, f"{HIST_FORECAST_BASE}{HIST_FORECAST_PATH}", params)
    finally:
        if close: await client.aclose()

# ---- Convenience bundle -----------------------------------------------------
async def get_weather_bundle(
    lat: float, lon: float, *, timezone: str = "auto", days: int = 3, client: Optional[httpx.AsyncClient] = None
) -> Dict[str, Any]:
    close = False
    if client is None:
        client, close = build_client(), True
    try:
        t1 = asyncio.create_task(get_current_weather(lat, lon, timezone=timezone, client=client))
        t2 = asyncio.create_task(get_daily_forecast(lat, lon, days=days, timezone=timezone, client=client))
